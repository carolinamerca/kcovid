...
library(spData)
##install.packages(c("cowplot", "googleway", "ggrepel", "ggspatial", "rnaturalearth", "rnaturalearthdata", "rgeos"))
library(cowplot)
library(googleway)
library(ggrepel)
library(ggspatial)
...
###Os valores negativos significam que houve uma diminuição na percentagem de pessoas com determinado sinal clinico
...
ggplot(incidencia_melt, aes(x=Data, y=Valores*100, color=Género)) +
  geom_line() +
  labs(title="Incidência Diária por Género", color="") +
